...
function InteractionsByFunction({ id }: { id: ArweaveTransactionID }) {
  ...
  const [data, setData] =
    useState<{ height: number; interactions: number }[]>();
  ...
  function generateData(interactions: ContractInteraction[]) {
    ...
    for (const interaction of sortedInteractions) {
      ...
      if (
        ...
        cumulativeInteractions[cumulativeInteractions.length - 1].height ===
          interaction.height
      ) {
        continue;
      }
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: (ctx) => {
                return `Interaction Percentage: ${(
                  (functionData[ctx.label] / interactions.length) *
                  100
                ).toPrecision(5)}%`;
              },
            },
